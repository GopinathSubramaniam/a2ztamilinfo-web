...
###SITEURL###'
	...
	function getCustomRecentPosts()
	{
		...
		/* $stringVal = "<!-- wp:embed {\"url\":\"https://www.youtube.com/embed/P7Y7N7vBFf4\",\"type\":\"rich\",\"providerNameSlug\":\"embed-handler\",\"responsive\":true,\"className\":\"wp-embed-aspect-16-9 wp-has-aspect-ratio\"} -->\n<figure class=\"wp-block-embed is-type-rich is-provider-embed-handler wp-block-embed-embed-handler wp-embed-aspect-16-9 wp-has-aspect-ratio\"><div class=\"wp-block-embed__wrapper\">\nhttps://www.youtube.com/embed/P7Y7N7vBFf4\n</div></figure>\n<!-- /wp:embed -->";
		$val = get_string_between($stringVal, '<!-- wp:embed', '-->');
		echo $val;
		exit();
 */
		$pageNo = $_GET['pageNo'];
		...
		foreach ($posts as $post) {
			if (str_contains($post->post_content, "embed")) {
				$obj = get_string_between($post->post_content, '<!-- wp:embed', '-->');
				$json = json_decode($obj);
				$post->post_content = $json->url;
			}
			$post->img_url = wp_get_attachment_url( get_post_thumbnail_id($post->ID) );
		}
